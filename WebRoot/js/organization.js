...
function OrgShow(OrgNode_){
	...
	this.Run=function(){
		...
		//*************************** 由上到下 修正合并后的左距离 @sven 添加
		for(var n=1; n<=this.Depth; n++){
			var DepthNodes=this.DepthGroup[n].Nodes;
			for(var m=0; m<DepthNodes.length; m++){
					if((DepthNodes[m].Left +DepthNodes[m].Width) > maxLeft){
						maxLeft = DepthNodes[m].Left +  DepthNodes[m].Width;
					}
					if(DepthNodes[m].parentNodes.length > 1){//发现合并
						var oldLeft = DepthNodes[m].Left;
						
						DepthNodes[m].Left = (DepthNodes[m].parentNodes[0].Left+DepthNodes[m].parentNodes[0].Width/2+DepthNodes[m].parentNodes[DepthNodes[m].parentNodes.length-1].Left+DepthNodes[m].parentNodes[DepthNodes[m].parentNodes.length-1].Width/2)/2-DepthNodes[m].Width/2;			
						if(DepthNodes[m].Nodes && DepthNodes[m].Nodes.length > 0){
							adjustLeft(DepthNodes[m].Nodes,DepthNodes[m].Left - oldLeft);
						}
						
					}
			}
		}
		if(maxLeft < jQuery(window).width()){
			this.Left = (jQuery(window).width() - maxLeft)/2;
		}
		...
		for(var n=1; n<=this.Depth; n++){//设置竖线条
			...
			for(var m=0; m<tempNodes.length; m++){
				if(n!=this.Depth){//设置底线条
					if(tempNodes[m].Nodes.length!=0 || tempNodes[m].singleChildId){
						var tempLineLeft=tempNodes[m].Left + (tempNodes[m].Width / 2);
						var tempLineHeight=((this.IntervalHeight - this.LineSize) / 2) + this.LineSize;
		...
		for(var n=2; n<=this.Depth; n++){//设置横线条
			...
			for(var m=0; m<tempNodeGroups_.length; m++){
				...
				}else if(tempNodeGroups_[m].length==1 && tempNodeGroups_[m][0].parentNodes.length > 1){//如果是合并的情况@sven 添加
					var tempLineWidth=tempNodeGroups_[m][0].parentNodes[tempNodeGroups_[m][0].parentNodes.length-1].Left -tempNodeGroups_[m][0].parentNodes[0].Left + tempNodeGroups_[m][0].parentNodes[tempNodeGroups_[m][0].parentNodes.length-1].Width/2 - tempNodeGroups_[m][0].parentNodes[0].Width/2;
					var tempLineLeft=tempNodeGroups_[m][0].parentNodes[0].Left + (tempNodeGroups_[m][0].parentNodes[0].Width / 2)+this.LineSize;
					var tempLineTop=tempNodeGroups_[m][0].Top - ((this.IntervalHeight - this.LineSize) / 2) - this.LineSize;
					var tempGroupLine=new CreateLine(1, tempLineTop, tempLineLeft, tempLineWidth, this.LineSize, this.LineColor, "LineGroup_"+ tempNodeGroups_[m][0].Id);
				}
			}
	...
	function adjustLeft(Nodes_,ErrDistance_){//修正合并后的子节点的错误位置
		for(var i = 0; i < Nodes_.length;i++){
			Nodes_[i].Left = Nodes_[i].Left + ErrDistance_;
			if(Nodes_[i].Nodes && Nodes_[i].Nodes.length > 0){
				adjustLeft(Nodes_[i].Nodes,ErrDistance_);
			}
		}
	}

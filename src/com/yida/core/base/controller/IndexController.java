...
public class IndexController extends BaseController {
	...
	public String login() {
		try {
			// 检查账号登录的合法性
			Account loginAccount = accountService.handleLogin(loginForm);
			
			// 检查账号的有效性
			if (loginAccount.getInvalid()) {
				throw new IllegalStateException("账号被挂起，暂不能登录。");
			}
			
			if (null == loginAccount.getType()) {
				throw new IllegalStateException("不能确定账号类型。");
			}
			...
			if (Account.Type.STAFF == loginAccount.getType()) {
				...
				if (null == loginStaff || !StringUtils.isMeaningFul(loginStaff.getId())) {
					throw new IllegalStateException("账号没有关联相关人员，暂不能登录。");
				}
				
				// 判断该账号关联的人员是否有效
				if (loginStaff.getInvalid()) {
					throw new IllegalStateException("账号关联的人员记录被标记为无效状态，暂不能登录。");
				}
				
				// 确定该账号所属的机构
				AuditOrg auditOrg = staffService.getAuditOrgForStaff(loginStaff);
				if (null == auditOrg) {
					throw new IllegalStateException("不能确定该账号所属的机构。");
				}
	...
	public String unlockAccount() {
		...
		try {
			Account loginAccount = super.getCurrentAccount();
			if (null != loginForm) {
				if (!loginAccount.getPassword().equals(loginForm.getPassword())) {
